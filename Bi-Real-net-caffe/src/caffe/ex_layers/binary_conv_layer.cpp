...
template <typename Dtype>
void BinaryConvolutionLayer<Dtype>::ModifyGradientsCPUTo(const shared_ptr<Blob<Dtype> > dw_b, const shared_ptr<Blob<Dtype> > w_r, const shared_ptr<Blob<Dtype> > dw_r) {
...
template <typename Dtype>
void BinaryConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
    const Dtype* weights, Dtype* output, bool skip_im2col) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
    if (!skip_im2col) {
...
template <typename Dtype>
void BinaryConvolutionLayer<Dtype>::forward_gpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void BinaryConvolutionLayer<Dtype>::backward_gpu_gemm(const Dtype* output,
    const Dtype* weights, Dtype* input) {
...
template <typename Dtype>
void BinaryConvolutionLayer<Dtype>::weight_gpu_gemm(const Dtype* input,
    const Dtype* output, Dtype* weights) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
    conv_im2col_gpu(input, col_buffer_.mutable_gpu_data());
...
template <typename Dtype>
void BinaryConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    const Dtype* input) {
